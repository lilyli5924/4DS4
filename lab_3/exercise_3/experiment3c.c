...
void bubble_sort_short(char *data_array, int size)
{
	...
	for (i=0; i<size-j-1; i++) {
		...
		//printf("data_array[%d] = %d, i-1= %d, i+1= %d, i+2 = %d \n", i, arr_var_1, data_array[i-1], arr_var_2, arr_var_3);
		//printf("lead = %d, end = %d ", lead, end);
		sec_1 = arr_var_1 & mask(lead);
		sec_2 = (arr_var_2 >> (lead - 1)) & mask(end);
		//printf("sec_1 = %d, sec_2 = %d. \n",sec_1, sec_2);
		...
		if (lead != 1){
			...
			// special case, only need to shift by 1 for when end == 8
			var2 = ((sec_3 << 1) + sec_4);
		}
		//sign extension
		var1_ext = (var1 & 0x1FF) | ((var1 & 0x100) ? 0xFE00 : 0);
		var2_ext = (var2 & 0x1FF) | ((var2 & 0x100) ? 0xFE00 : 0);
		//printf("var1_ext = %d, var2_ext = %d \n", var1_ext, var2_ext);
