...
extern OS_EVENT *GRPAMutex; // SW0 ~ SW8
extern OS_EVENT *GRPBMutex; // SW9 ~ SW16
extern OS_EVENT *REDMutex;
extern OS_EVENT *GREENMutex;
...
int main(void) {
    ...
    alt_up_character_lcd_string(lcd_0, "COE4DS4 Winter20");
...
int init_OS_data_structs(void) {
	...
	GRPAMutex = OSMutexCreate(SWITCH_A_MUTEX_PRIORITY, &return_code);
	GRPBMutex = OSMutexCreate(SWITCH_B_MUTEX_PRIORITY, &return_code);
	REDMutex = OSMutexCreate(RED_LED_MUTEX_PRIORITY, &return_code);
	GREENMutex = OSMutexCreate(GREEN_LED_MUTEX_PRIORITY, &return_code);
